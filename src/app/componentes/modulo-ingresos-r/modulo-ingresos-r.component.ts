...
@Component({
  selector: 'app-modulo-ingresos-r',
  templateUrl: './modulo-ingresos-r.component.html',
  styleUrls: ['./modulo-ingresos-r.component.css']
})
export class ModuloIngresosRComponent implements OnInit {
  ingresos: any[] = [];
  tiposIngreso: any[] = [];
  pacientes: any[] = [];
  formasPago: any[] = [];
  ...
  agregarOActualizarIngreso(): void {
    if (this.ingresoForm.valid) {
      const ingreso = {
        ...
        fecha_inicio: moment(this.ingresoForm.value.fecha_inicio).format('YYYY-MM-DD HH:mm:ss'),
      ...
      if (this.editMode) {
        ...
        this.ingresosService.agregarIngreso(ingreso).subscribe(() => {
          this.resetForm();
          this.obtenerIngresos();
        });
      }
